...
window = glfw.create_window(800, 600, "Colisão com Modo Debug", None, None)
if not window:
    glfw.terminate()
...
# Função callback para teclas
def key_callback(window, key, scancode, action, mods):
    global modo_debug
    if key == glfw.KEY_B and action == glfw.PRESS:
        modo_debug = not modo_debug  # Alterna o modo debug
        print(f"Modo Debug: {'ATIVADO' if modo_debug else 'DESATIVADO'}")

# Registra o callback de teclado
...
# Função para desenhar caixas de colisão no modo debug
def desenhar_caixa_colisao(x, y, tamanho, cor=(1.0, 1.0, 0.0)):
    """Desenha a caixa envolvente do jogador com linhas amarelas"""
    glColor3f(cor[0], cor[1], cor[2])
    glLineWidth(2.0)  # Linhas mais grossas para melhor visualização
...
while not glfw.window_should_close(window):
    ...
    # === MODO DEBUG: Renderiza as caixas de colisão como linhas ===
    if modo_debug:
        # Desenha caixas de colisão em cima do que já existe
        desenhar_caixa_colisao(pos_jogador[0], pos_jogador[1], tamanho)

    # Desenha a barreira e o jogador (sempre visíveis)
    desenhar_barreira()
