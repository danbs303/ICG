...
try:
    scene = pywavefront.Wavefront('boneco.obj', collect_faces=True, create_materials=True)
...
def draw_scene(window):
    ...
    gluLookAt(8, 8, 8, 0, 2, 0, 0, 1, 0)

    # Corzinha pro dado (lilás)
    glMaterialfv(GL_FRONT, GL_DIFFUSE, [0.6, 0.5, 0.9, 1.0])
...
def main():
    ...
    window = glfw.create_window(800, 600, "TPose", None, None)
    if not window:
        glfw.terminate()
        return
